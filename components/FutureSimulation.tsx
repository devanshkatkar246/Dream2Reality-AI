      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
        ...
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
            <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex flex-col gap-3 min-w-[200px]">
              <div className="flex items-center gap-3">
                <Loader2 size={16} className="text-primary animate-spin" />
                <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Generating Scenario</span>
              </div>
              <div className="space-y-2 w-full animate-pulse mt-1">
                <div className="h-2.5 bg-white/10 rounded w-full"></div>
                <div className="h-2.5 bg-white/10 rounded w-[85%]"></div>
                <div className="h-2.5 bg-white/10 rounded w-[40%]"></div>
              </div>
            </div>
          </motion.div>
        )}
